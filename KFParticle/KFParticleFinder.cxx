...
KFParticleFinder::KFParticleFinder() : fNPV(-1), fNThreads(1), fDistanceCut(1.f), fLCut(-5.f), fCutCharmPt(0.2f), fCutCharmChiPrim(85.f), fCutLVMPt(0.0f), fCutLVMP(0.0f), fCutJPsiPt(1.0f), fD0(0), fD0bar(0), fD04(0), fD04bar(0), fD0KK(0), fD0pipi(0), fDPlus(0), fDMinus(0), fDPlus3Pi(0), fDMinus3Pi(0), fDsPlusK2Pi(0), fDsMinusK2Pi(0), fLcPlusP2Pi(0), fLcMinusP2Pi(0), fLPi(0), fLPiPIndex(0), fHe3Pi(0), fHe3PiBar(0), fHe4Pi(0), fHe4PiBar(0), fHe4L(0), fHe5L(0), fLLn(0), fH5LL(0), fSecCandidates(), fPrimCandidates(), fPrimCandidatesTopo(), fPrimCandidatesTopoMass(), fEmcClusters(0), fMixedEventAnalysis(0), fDecayReconstructionList()
{
  ...
  fCutsTrackV0[0][0] = 5;
  fCutsTrackV0[0][1] = 5;
  fCutsTrackV0[0][2] = 6; //Xi, Omega
  fCutsTrackV0[1][0] = 5;
  fCutsTrackV0[1][1] = 5;
  fCutsTrackV0[1][2] = 6; //Charm, H0, Sigma+
  fCutsTrackV0[2][0] = -100.;
  fCutsTrackV0[2][1] = 10000.;
  fCutsTrackV0[2][2] = 3; //resonances

  //charm
  //chi2               l/dl                  chi2_topo
  fCutsCharm[0] = 3.f;
  fCutsCharm[1] = 10.f;
  fCutsCharm[2] = 3.f; //D0 -> pi+ K-

  //cuts on particles reconstructed from short-lived particles
  //ldl,                      chi2_topo                 chi2_geo
  //H0 -> Lambda Lambda, Xi0 -> Lambda pi0
  fCutsPartPart[0][0] = 10;
  fCutsPartPart[0][1] = 3;
  fCutsPartPart[0][2] = 3;
  //Sigma0 -> Lambda Gamma, pi0 -> Gamma Gamma, K* -> K pi0, Sigma*0 -> Lambda pi0, Xi* -> Xi pi0
  fCutsPartPart[1][0] = -10;
  fCutsPartPart[1][1] = 3;
  fCutsPartPart[1][2] = 3;
}
  ...
  for (int iTrTypeNeg = 0; iTrTypeNeg < 2; iTrTypeNeg++) {
    ...
    for (int iTrTypePos = 0; iTrTypePos < 2; iTrTypePos++) {
      ...
      if ((iTrTypeNeg == 0) && (iTrTypePos == 0)) {
        ...
        // e-
        startTCPos[0] = 0;
        endTCPos[0] = nPositiveTracks; //posTracks.LastElectron();
        startTCNeg[0] = 0;
        endTCNeg[0] = negTracksSize[0]; //negTracks.LastElectron();
        //mu-
        startTCPos[1] = 0;
        endTCPos[1] = 0;
        startTCNeg[1] = 0;
        endTCNeg[1] = 0;
        //pi- + ghosts
        startTCPos[2] = posTracks.FirstPion();
        endTCPos[2] = nPositiveTracks;
        startTCNeg[2] = negTracks.FirstPion();
        endTCNeg[2] = negTracks.LastPion();
        //K-
        startTCPos[3] = posTracks.FirstPion();
        endTCPos[3] = posTracks.LastKaon();
        startTCNeg[3] = negTracks.FirstKaon();
        endTCNeg[3] = negTracks.LastKaon();
        //p-, d-, t-, he3-, he4-
        startTCPos[4] = posTracks.FirstPion();
        endTCPos[4] = posTracks.LastPion();
        startTCNeg[4] = negTracks.FirstProton();
        endTCNeg[4] = negTracksSize[0];
      }
      ...
      if ((iTrTypeNeg == 1) && (iTrTypePos == 1)) {
        ...
        endTCNeg[0] = negTracksSize[0]; //negTracks.LastElectron();
        //mu-
        startTCPos[1] = posTracks.FirstMuon();
        endTCPos[1] = posTracks.LastMuon();
        startTCNeg[1] = negTracks.FirstMuon();
        endTCNeg[1] = negTracks.LastMuon();
        //pi- + ghosts
        startTCPos[2] = posTracks.FirstPion();
        endTCPos[2] = posTracks.LastProton();
        startTCNeg[2] = negTracks.FirstPion();
        endTCNeg[2] = negTracks.LastPion();
        //K-
        startTCPos[3] = posTracks.FirstPion();
        endTCPos[3] = nPositiveTracks;
        startTCNeg[3] = negTracks.FirstKaon();
        endTCNeg[3] = negTracks.LastKaon();
        //p-
        startTCPos[4] = posTracks.FirstPion();
        endTCPos[4] = posTracks.LastProton();
        startTCNeg[4] = negTracks.FirstProton();
        endTCNeg[4] = negTracks.LastProton();
      }
        ...
        for (int iTrN = startTCNeg[iTC]; iTrN < endTCNeg[iTC]; iTrN += float_vLen) {
          ...
          for (int iTrP = startTCPos[iTC]; iTrP < endTCPos[iTC]; iTrP += float_vLen) {
            ...
            for (int iRot = 0; iRot < float_vLen; iRot++) {
              ...
              if (closeDaughters.isEmpty() && (iTC != 0))
                continue;
              ...
              for (int iPDGPos = 0; iPDGPos < nPDGPos; iPDGPos++) {
                ...
                if (!fMixedEventAnalysis) {
                  if (iTC == 0) {
                    ...
                    motherPDG(isPrimary && (abs(trackPdgPos[iPDGPos]) == 13 || abs(trackPdgPos[iPDGPos]) == 19) && (int_m(abs(trackPdgNeg) == 13) || int_m(abs(trackPdgNeg) == 19))) = 200113; //rho -> mu+ mu-
                  } else if (iTC == 2) {
                                            ...
                                            std::vector<KFParticle>* vMotherSec)
{
  ...
  for (unsigned int iv = 0; iv < nElements; iv++) {
    ...
    if (abs(mother.GetPDG()[iv]) == 521 || abs(mother.GetPDG()[iv]) == 529 || abs(mother.GetPDG()[iv]) == 511 || abs(mother.GetPDG()[iv]) == 519) {
      ...
      if ((fabs(mass - massPDG) / massSigmaPDG) > 3)
        continue;
    ...
    if (vMotherPrim) {
      if (!((abs(mother.GetPDG()[iv]) == 3312) || (abs(mother.GetPDG()[iv]) == 3324)))
        continue;
      float mass, errMass;
                                        ...
                                        vector<KFParticle>* vMotherSec)
{
  ...
  if ((vV0.size() < 1) || ((lastTrack - firstTrack) < 1))
    return;
  KFParticle mother_temp;
                                       ...
                                       const float& massCut)
{
  ...
  for (int iC = 0; iC < nCand; iC += float_vLen) {
    ...
    for (int iv = 0; iv < nEntries; iv++) {
      ...
      bool isPrimary = 0;
      for (int iP = 0; iP < fNPV; iP++) {
        if (!(KFPMath::Finite(candTopo[iP].GetChi2())[iv]))
          continue;
        if (!(candTopo[iP].GetChi2()[iv] > 0.0f))
          continue;
        if (!(candTopo[iP].GetChi2()[iv] == candTopo[iP].GetChi2()[iv]))
          continue;
      ...
      if ((fabs(m - mass) / massErr) > massCut)
        continue;
                                       ...
                                       float massMotherPDGSigma)
{
  ...
  if ((particles1.size() == 0) || (particles2.size() == 0))
    return;
  if (!(fDecayReconstructionList.empty()) && (fDecayReconstructionList.find(MotherPDG) == fDecayReconstructionList.end()))
    return;
    ...
    for (int iP2 = startIndex; iP2 < nPart2; iP2 += float_vLen) {
      ...
      for (int iv = 0; iv < nElements; iv++) {
        ...
        if (vMotherPrim || vMotherSec) {
          float mass, errMass;
          mother_temp.GetMass(mass, errMass);
          if ((fabs(mass - massMotherPDG) / massMotherPDGSigma) > 3.f)
            continue;
                                            ...
                                            vector<KFParticle>& Particles)
{
  ...
  for (int iTrTypeDaughter = 0; iTrTypeDaughter < 2; iTrTypeDaughter++) {
    ...
    //mu+, mu-
    startTCMother[0] = 0;
    endTCMother[0] = MotherTracksSize;
    startTCDaughter[0] = DaughterTracks.FirstMuon();
    endTCDaughter[0] = DaughterTracks.LastMuon();
    ...
    //Pi+, Pi-
    startTCMother[1] = 0;
    endTCMother[1] = MotherTracksSize;
    startTCDaughter[1] = DaughterTracks.FirstPion();
    endTCDaughter[1] = DaughterTracks.LastPion();
    ...
    //K+, K-
    startTCMother[2] = 0;
    endTCMother[2] = MotherTracksSize;
    startTCDaughter[2] = DaughterTracks.FirstKaon();
    endTCDaughter[2] = DaughterTracks.LastKaon();
    ...
    //p+, p-
    startTCMother[3] = 0;
    endTCMother[3] = MotherTracksSize;
    startTCDaughter[3] = DaughterTracks.FirstProton();
    endTCDaughter[3] = DaughterTracks.LastProton();
          ...
          for (int iRot = 0; iRot < float_vLen; iRot++) {
            ...
            for (int iHypothesis = 0; iHypothesis < nMotherHypothesis[iTC]; iHypothesis++) {
              ...
              if (iTrTypeDaughter == 0)
                motherKFPDG = -outMotherPDG[iTC][iHypothesis];
              if (!(fDecayReconstructionList.empty()) && (fDecayReconstructionList.find(motherKFPDG) == fDecayReconstructionList.end()))
                continue;
...
void KFParticleFinder::AddCandidate(const KFParticle& candidate, int iPV)
{
  ...
  int iSet = -1;
  if (candidate.GetPDG() == 310)
    iSet = 0;
  if (candidate.GetPDG() == 3122)
    iSet = 1;
  if (candidate.GetPDG() == -3122)
    iSet = 2;
  if (candidate.GetPDG() == 22)
    iSet = 3;
  if (candidate.GetPDG() == 111)
    iSet = 4;
  if (candidate.GetPDG() == 3312)
    iSet = 5;
  if (candidate.GetPDG() == -3312)
    iSet = 6;
  if (candidate.GetPDG() == 3334)
    iSet = 7;
  if (candidate.GetPDG() == -3334)
    iSet = 8;
