...
static inline __attribute__((always_inline)) float_v ATan2(const float_v& y, const float_v& x)
{
  ...
  b += (((8.05374449538e-2f * a2 - 1.38776856032E-1f) * a2 + 1.99777106478E-1f) * a2 - 3.33329491539E-1f) * a2 * a + a;
  b(xNeg ^ yNeg) = -b;
  b(xNeg && !yNeg) = (b + pi);
  b(xNeg && yNeg) = (b - pi);
  b(xZero && yZero) = zero;
...
typename Vc::Vector<T>::Mask
  Finite(const Vc::Vector<T>& x)
{
  return Vc::isfinite(x);
}
template <typename T>
static inline __attribute__((always_inline)) T Log(const T& x)
{
  return std::log(x);
}
template <typename T>
static inline __attribute__((always_inline)) T ACos(const T& x)
{
  return (3.1415926535897f / 2.f - asin(x));
}
} // namespace KFPMath
